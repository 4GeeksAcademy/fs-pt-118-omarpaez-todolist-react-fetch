    ...
    return (
        <>
            <li className={`d-flex justify-content-between list-group-item text-start  ${isHover ? 'active' : ''}`}
                ...
                key={id}>
                <span>
                    {label} <i className={`fa-solid ${done ? "fa-circle-check text-primary-emphasis" : "fa-solid fa-square-xmark text-danger"}`}/>
                       
                </span>
                {isHover &&
                        <div className="me-2">
                            <div className={`btn ${done ? "btn-danger" : "btn-success"} btn-sm shadow border border-light`} onClick={() => { handleDone() }}>{done ? "No Hecho" : "Hecho"}</div>
                            ...
                            <span className="pointer" onClick={() => { onSelectDelete(id) }} >
                                <i className="fa-solid fa-trash ms-2"></i></span>
                        </div>}
            </li>
            {editTaskId === id &&
                <EditTask
